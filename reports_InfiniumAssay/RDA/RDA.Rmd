...
env <- env %>% 
  dplyr::select(clon,prov,contains("ude_prov"),contains("WC"),contains("top_prov"),"depth_roots_prov", paste0(rep("Q",6),1:6),max.Q)
colnames(env) <- c("genotype","prov",
                   str_sub(grep("ude_prov",colnames(env),value=TRUE),1,-6),
                   str_sub(grep("bio",colnames(env),value=TRUE),1,-8),
                   str_sub(grep("top_prov",colnames(env),value=TRUE),1,-6),
                   str_sub(grep("roots_prov",colnames(env),value=TRUE),1,-6),
                   paste0(rep("Q",6),1:6),"maxgp")
env <- unique(env)
...
env <- merge(env,topo,by="prov") %>% 
  merge(readRDS(file="data/FireData/DfBurnedAreabyProv.rds") %>% dplyr::select(BurnedArea,prov),by="prov")
...
```{r Set1,fig.height=9,fig.width=12}
select.var.AvgSand <- c("bio1","bio2","bio12","bio14","sand_top","depth_roots","TRI","BurnedArea")
pairs.panels(unique(env[,select.var.AvgSand]), scale=T,hist.col="palegreen1") # unique() here because there is one env. value per provenance
```
...
```{r Set2,fig.height=9,fig.width=12}
select.var.AvgWater <- c("bio1","bio2","bio12","bio14","water_top","depth_roots","TRI","BurnedArea")
pairs.panels(unique(env[,select.var.AvgWater]), scale=T,hist.col="palegreen1") # unique() here because there is one env. value per provenance
```
...
```{r Set3,fig.height=9,fig.width=12}
select.var.ExtSand <- c("bio5","bio6","bio12","bio15","sand_top","depth_roots","TRI","BurnedArea")
pairs.panels(unique(env[,select.var.ExtSand]), scale=T,hist.col="palegreen1")
```
...
```{r Set4,fig.height=9,fig.width=12}
select.var.ExtWater <- c("bio5","bio6","bio12","bio15","water_top","depth_roots","TRI","BurnedArea")
pairs.panels(unique(env[,select.var.ExtWater]), scale=T,hist.col="palegreen1")
```
...
```{r Set5,fig.height=9,fig.width=12}
select.var.ExtWater <- c("bio16",
                         "bio17",
                         "bio4",
                         "bio9",
                         "sand_top",
...
```{r Set6,fig.height=9,fig.width=12}
select.var.ExtWater <- c("bio16",
                         "bio17",
                         "bio4",
                         "bio9",
                         "water_top","depth_roots","TRI","BurnedArea")
pairs.panels(unique(env[,select.var.ExtWater]), scale=T,hist.col="palegreen1")
```
...
nvar <- length(select.var.AvgSand)
```
...
provs <- env %>% dplyr::select(prov,latitude,longitude) %>% distinct()
mem <- provs %>% dplyr::select(latitude,longitude) %>% dbmem(MEM.autocor = c("positive"),store.listw = FALSE)
...
correctionmem.rda <- rda(gen.imp ~ bio1 + bio2 + bio12 + bio14 + water_top + depth_roots + TRI + BurnedArea + Condition(MEM1 + MEM2 + MEM3 + MEM4),
...
correctiongp.rda <- rda(gen.imp ~ bio1 + bio2 + bio12 + bio14 + water_top + depth_roots + TRI + BurnedArea + Condition(Q1 + Q2 + Q3 + Q4 + Q4 + Q5), data=env[,c(select.var.AvgWater,paste0(rep("Q",5),1:5))], scale=T)
...
par(mfrow=c(1,4))
load.correctiongp.rda <- scores(correctiongp.rda, choices=c(1:nvar), display="species")  # Species scores for the seven constrained axes (all significant)
...
cand2 <- outliers(load.correctiongp.rda[,2],3) # 25 candidate SNPs on axis 2
cand3 <- outliers(load.correctiongp.rda[,3],3) # 27 candidate SNPs on axis 3
cand4 <- outliers(load.correctiongp.rda[,4],3) # 32 candidate SNPs on axis 4
cand5 <- outliers(load.correctiongp.rda[,5],3) # 19 candidate SNPs on axis 5
cand6 <- outliers(load.correctiongp.rda[,6],3) # 23 candidate SNPs on axis 6
cand7 <- outliers(load.correctiongp.rda[,7],3) # 22 candidate SNPs on axis 7
cand8 <- outliers(load.correctiongp.rda[,8],3) # 21 candidate SNPs on axis 8

ncand <- length(cand1) + length(cand2) + length(cand3) + length(cand4) + length(cand5) + length(cand6) + length(cand7) + length(cand8)
ncand
```

> <span style="color: red;">221 candidates SNPs in total.</span>
...
Now we have a data frame of 221 candidate SNPs and their correlation with our 7 environmental predictors.
...
> 5 duplicates, 216 remaining candidate SNPs.
...
> There are 140 drought-associated candidate SNPs with a coeff higher than 0.1 and 20 with a beta coeff higher than 0.3.
...
> There are 137 drought-associated candidate SNPs.
...
correctiongp.rda <- rda(gen.imp ~ bio1 + bio2 + bio12 + bio14 + sand_top + depth_roots + TRI + BurnedArea + Condition(Q1 + Q2 + Q3 + Q4 + Q4 + Q5), data=env[,c(select.var.AvgSand,paste0(rep("Q",5),1:5))], scale=T)
correctiongp.rda
RsquareAdj(correctiongp.rda)
summary(eigenvals(correctiongp.rda, model = "constrained"))
screeplot(correctiongp.rda)
...
> 3 duplicates, 208 remaining candidate SNPs.
...
DF <- data.frame(threshold=c(0.1,0.3), 
                 ...
                 bio14=c(nrow(cand[abs(cand$bio14)>0.1,]),
                        nrow(cand[abs(cand$bio14)>0.3,])),
                 sand_top=c(nrow(cand[abs(cand$sand_top)>0.1,]),
                        nrow(cand[abs(cand$sand_top)>0.3,])),
                 depth_roots=c(nrow(cand[abs(cand$depth_roots)>0.1,]),
                        nrow(cand[abs(cand$depth_roots)>0.3,])),
                 TRI=c(nrow(cand[abs(cand$TRI)>0.1,]),
                        nrow(cand[abs(cand$TRI)>0.3,])),
...
correctiongp.rda <- rda(gen.imp ~ bio5 + bio6 + bio13 + bio14 + sand_top + depth_roots + TRI + BurnedArea +Condition(Q1 + Q2 + Q3 + Q4 + Q4 + Q5), data=env[,c(select.var.ExtSand,paste0(rep("Q",5),1:5))], scale=T)
correctiongp.rda
RsquareAdj(correctiongp.rda)
summary(eigenvals(correctiongp.rda, model = "constrained"))
screeplot(correctiongp.rda)
...
> <span style="color: red;">210 candidates SNPs in total.</span>
...
> 1 duplicate, 209 remaining candidate SNPs.
...
```{r SumUpCandidateNumberExtSand}
DF <- data.frame(threshold=c(0.1,0.3), 
                 bio13=c(nrow(cand[abs(cand$bio13)>0.1,]),
                        nrow(cand[abs(cand$bio13)>0.3,])),
...
correctiongp.rda <- rda(gen.imp ~ bio5 + bio6 + bio13 + bio14 + water_top + depth_roots + TRI + BurnedArea +Condition(Q1 + Q2 + Q3 + Q4 + Q4 + Q5), data=env[,c(select.var.ExtWater,paste0(rep("Q",5),1:5))], scale=T)
correctiongp.rda
RsquareAdj(correctiongp.rda)
summary(eigenvals(correctiongp.rda, model = "constrained"))
screeplot(correctiongp.rda)
...
```{r SumUpCandidateNumberExtWater}
DF <- data.frame(threshold=c(0.1,0.3), 
                 bio13=c(nrow(cand[abs(cand$bio13)>0.1,]),
                        nrow(cand[abs(cand$bio13)>0.3,])),
                 bio5=c(nrow(cand[abs(cand$bio5)>0.1,]),
