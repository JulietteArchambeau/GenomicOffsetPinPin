---
title: "Estimating the genetic-environment relationship using the whole genomic dataset"
...
PlotCors(data,select.var = c(select.var.AvgSand,"latitude","longitude"))
```
...
PlotCors(data,select.var = c(select.var.AvgWater,"latitude","longitude"))
```
...
PlotCors(data,select.var = c(select.var.ExtSand,"latitude","longitude"))
```
...
PlotCors(data,select.var = c(select.var.ExtWater,"latitude","longitude"))
```
...
PinpinDistri  <- shapefile('data/maps/MaskPinpinDistri/PinpinDistri.shp')
```
...
stackall.lowres <- stack("data/StacksEnvVars/CurStacksResWorldClim/StackAvgWaterResWorldClim.grd")
```
...
## Distance-based Moran Eigen vectors



```{r CalculateMoranEigenVectors, eval=F}
# Extract the values in the buffer corresponding to the maritime pine distribution
# The values are extracted from the environmental rasters at low resolution (~31km² resolution) because the computer can't provide enough memory to calculate the moran eigenvectors at high resolution. 
rasValue <- raster::extract(stackall.lowres, PinpinDistri,cellnumbers=T)
rasValue <- as.data.frame(rasValue) # 12982 values
#rasValue <- na.omit(rasValue) # no need to run this line as there is no NAs

###
# At ~1km² resolution: not possible because of memory space:
# Error message: "Erreur : impossible d'allouer un vecteur de taille 391.6 Go" when trying to calculate the MEM for all values
###


# We join the cell number with the x=longitude and y=latitude
ex.df.coords <- cbind(xyFromCell(stackall.lowres,rasValue[,1]),rasValue)

# And we add the 34 provenances!
ex.df.coords <- ex.df.coords %>% rename(latitude=y,longitude=x)  %>% select(latitude,longitude) %>% 
  rbind(data[,c("latitude","longitude")])

# Here we calculate the distance-based Moran eigen vectors:
moraneigenvectors <- ex.df.coords %>% dbmem
# There are 3,708 eigenfunctions with positive eigenvalues!

# Save all the MEMs
saveRDS(moraneigenvectors,file="outputs/GF/moraneigenvectors/moraneignevectorsAllDim.rds")

# Save the MEMs of the provenances only and 4 dimensions only. 34 values.
saveRDS(moraneigenvectors[(13016-33):13016,1:4],file="outputs/GF/moraneigenvectors/moraneignevectorsProvs.rds")

# Save the MEMs of the cells in the buffer (maritime pine distribution). 12982 values.
saveRDS(moraneigenvectors[0:(13016-34),1:4],file="outputs/GF/moraneigenvectors/moraneignevectorsPinpinDistri.rds")
```
...
```{r LoadFstMatrix}
fstmat <- readRDS(file="data/PreparedDFforGDMandGF/GDM/FstMatrices/FstMatrix5165Snps34Provs_noNeg.rds")
...
In our case, there are four MEM eigenfunctions with positive eigenvalues, we selected the four. 

```{r MoranEigenVectors}
mem <- readRDS(file="outputs/GF/moraneigenvectors/moraneignevectorsProvs.rds")
...
gf.mem <- readRDS(file=paste0("outputs/GF/models/GFAvgWater.rds"))
```
...
gf.geo <- readRDS(file=paste0("outputs/GF/models/GFAvgWaterGeo.rds"))
```


```{r PlotOverallImptanceGFAvgWaterGeo,cache=TRUE, fig.height=4,fig.width=8}
plot(gf.geo, plot.type="Overall.Importance")
mean(gf.geo$result)
sd(gf.geo$result)
```

```{r PlotCumulativeImptanceGFAvgWaterGeo,cache=TRUE,fig.height=4,fig.width=4}
# Latitude
plot(gf.geo, plot.type="Cumulative.Importance",
     imp.vars="latitude",
     plot.args=list(show.species=FALSE,show.overall=TRUE))
...
## Projections under current climate

### GDM

```{r TransformingBeforeProjectionGDMCurrentClimate,cache=TRUE}
...
```{r BiplotGDMCurrentClimate, fig.height=8, fig.width=8,cache=T}
...
Calculating the Moran eigen vectors on the rasters at ~1km² resolution was not possible on my laptop as I did not have enough memory. That's why we did the projections of the rasters at ~ 31km² (2.5 minutes spatial resolution). 

```{r TransformingBeforeProjectionGFMEMCurrentClimate,cache=TRUE}
rasValue <- raster::extract(stackall.lowres, PinpinDistri,cellnumbers=T)
rasValue <- as.data.frame(rasValue)
rasValue <- na.omit(rasValue)
...
# So we project at ~31km² resolution:
#ex.df.coords <- cbind(xyFromCell(stackall.lowres,rasValue[,1]),rasValue)
#ex.df.coords <- ex.df.coords %>% rename(latitude=x,longitude=y)
#moraneigenvectors <- ex.df.coords %>% select(latitude,longitude) %>% dbmem
#saveRDS(moraneigenvectors,file="outputs/GF/moraneigenvectors/moraneignevectors.rds")
#saveRDS(moraneigenvectors[,1:4],file="outputs/GF/moraneigenvectors/moraneignevectors4Dim.rds")
moraneigenvectors <- readRDS(file="outputs/GF/moraneigenvectors/moraneignevectorsPinpinDistri.rds")
rasValue <- cbind(moraneigenvectors,rasValue)
###

# transform env using gf models, see ?predict.gradientForest
projCur.mem <- predict(gf.mem, rasValue %>%  dplyr::select(-cell)) # remove cell column before transforming

# ppcatoraster by hand
pca <- prcomp(projCur.mem, center=TRUE, scale.=FALSE)
```


Comment about the `predict.gradientForest` function: adding or not the Moran eigen vectors in the function doesn't influence the biological importance of the other variables. Maybe it is more informative to not include the Moran eigen vectors here, as advised by Fitzpatrick and Keller. 


```{r ProjectionGFMEMCurrentClimate, fig.height=8, fig.width=8}
##assigns to colors, edit as needed to maximize color contrast, etc.
a1 <- pca$x[,1]; a2 <- pca$x[,2]; a3 <- pca$x[,3]
r <- a1+a2; g <- -a2; b <- a3+a2-a1
...
##assigns color to raster
rast1 <- rast2 <- rast3 <- stackall.lowres[[1]]
mapCells <- rasValue$cell
rast1[mapCells] <- scalR
rast2[mapCells] <- scalG
rast3[mapCells] <- scalB
...
```{r BiplotGFMEMCurrentClimate, echo=F,fig.height=6,fig.width=6, cache=T}
nvs <- dim(pca$rotation)[1]
vec <- names(gf.mem$X)
lv <- length(vec)
vind <- rownames(pca$rotation) %in% vec
...
PredsProvs <- predict(gf.mem)
PCprovs <- predict(pca, PredsProvs)
points(PCprovs[, 1:2])


# Show all the alleles
######################
SpsWtd <- sweep(gf.mem$Y, 2, apply(gf.mem$Y, 2, min),  "-")
...
Projection at ~1km2 spatial resolution.

```{r TransformingBeforeProjectionGFGEOCurrentClimateLatLong}
rasValue <- raster::extract(stackall, PinpinDistri,cellnumbers=T)
rasValue <- as.data.frame(rasValue)
rasValue <- na.omit(rasValue)
...
# transform env using gf models, see ?predict.gradientForest
projCur <- predict(gf.geo, rasValue[,-3]) # remove cell column before transforming

# ppcatoraster by hand
pca <- prcomp(projCur, center=TRUE, scale.=FALSE)
```

```{r ProjectionGFGEOCurrentClimateLatLong, fig.height=8, fig.width=8}
##assigns to colors, edit as needed to maximize color contrast, etc.
a1 <- pca$x[,1]; a2 <- pca$x[,2]; a3 <- pca$x[,3]
r <- a1+a2; g <- -a2; b <- a3+a2-a1
...
```{r BiplotGFGEOCurrentClimateLatLong, echo=F,fig.height=6,fig.width=6}
nvs <- dim(pca$rotation)[1]
vec <- names(gf.geo$X)
lv <- length(vec)
vind <- rownames(pca$rotation) %in% vec
...
# Show all the alleles
######################
SpsWtd <- sweep(gf.geo$Y, 2, apply(gf.geo$Y, 2, min),  "-")
SpsWtdPCs <- (t(SpsWtd) %*% (PCprovs[, 1:2]))/colSums(SpsWtd)
plot((pca$x[, 1:2]), xlim = xrng, ylim = yrng, pch = ".", cex = 3, col = rgb(scalR , scalG, scalB, max = 255), asp = 1)
points(SpsWtdPCs, pch = ".",cex=3,col="black") # col = alpha("black", 0.4)
...
## Projections under future climate
...
```{r ProjectionsGDMGCMsSSP370,fig.height=6,fig.width=6}
rastTrans <- gdm.transform(gdm.AvgWater, ssp370stackavg)
rastDat <-  na.omit(getValues(rastTrans))
rastDat <- as.data.frame(rastDat)

pca <- prcomp(rastDat, center=TRUE, scale.=FALSE) # bio5 does not appear as this variable has no explanatory power

# 1/ Following the GDM vignette (=> projection)
pcaRast <- predict(rastTrans, pca, index=1:3) # HERE
...
```{r TransformingBeforeProjectionGFMEMFutureClimateSSP370AvgWaterMEM}
rasValue <- raster::extract(ssp370stackavg, PinpinDistri,cellnumbers=T)
rasValue <- as.data.frame(rasValue)
...
# transform env using gf models, see ?predict.gradientForest
projFut <- predict(gf.mem, rasValue %>%  dplyr::select(-cell)) # remove cell column before transforming

# ppcatoraster by hand
pca <- prcomp(projFut, center=TRUE, scale.=FALSE) 
```

```{r ProjectionGFMEMFutureClimateSSP370LatLong, fig.height=8, fig.width=8}
##assigns to colors, edit as needed to maximize color contrast, etc.
a1 <- pca$x[,1]; a2 <- pca$x[,2]; a3 <- pca$x[,3]
r <- a1+a2; g <- -a2; b <- a3+a2-a1
...
```{r BiplotGFMEMFutureClimateSSP370LatLong, echo=F,fig.height=6,fig.width=6}
nvs <- dim(pca$rotation)[1]
vec <- names(gf.mem$X)
lv <- length(vec)
vind <- rownames(pca$rotation) %in% vec
...
```{r TransformingBeforeProjectionGFGEOFutureClimateSSP370LatLong}
rasValue <- raster::extract(ssp370stackavg, PinpinDistri,cellnumbers=T)
rasValue <- as.data.frame(rasValue)
rasValue <- na.omit(rasValue)

###
rasValue <- cbind(xyFromCell(ssp370stackavg,rasValue[,1]),rasValue)
rasValue <- rasValue %>% rename(longitude=x,latitude=y)
###

# transform env using gf models, see ?predict.gradientForest
projFut.geo <- predict(gf.geo, rasValue[,-3]) # remove cell column before transforming

# ppcatoraster by hand
pca <- prcomp(projFut.geo, center=TRUE, scale.=FALSE)
```

```{r ProjectionGFGEOFutureClimateSSP370LatLong, fig.height=8, fig.width=8}
##assigns to colors, edit as needed to maximize color contrast, etc.
a1 <- pca$x[,1]; a2 <- pca$x[,2]; a3 <- pca$x[,3]
r <- a1+a2; g <- -a2; b <- a3+a2-a1
...
```{r BiplotGFGEOFutureClimateSSP370LatLong, echo=F,fig.height=6,fig.width=6}
nvs <- dim(pca$rotation)[1]
vec <- names(gf.geo$X)
lv <- length(vec)
vind <- rownames(pca$rotation) %in% vec
...
GOAcrossRangeWithGDM <- function(x){
  ...
  rast.pred <- predict.gdm(gdm.AvgWater, stackall.lowres,time=TRUE, predRasts=x) 
...
```{r GOSSP370GFMEM, cache=T}
# `projCur.mem` is already in the same resolution (~31km2 spatial resolution) as the future climate projection.

GOAcrossRange <- function(x){
  
  rasValue <- raster::extract(x, PinpinDistri,cellnumbers=T)
  rasValue <- as.data.frame(rasValue)
  rasValue <- cbind(moraneigenvectors,rasValue)
  rasValue <- na.omit(rasValue)

  # transform env using gf models, see ?predict.gradientForest
  predFut <- predict(gf.mem, rasValue %>%  dplyr::select(-cell))
  
  # calculate euclidean distance between current and future genetic spaces  
  return(sqrt((projCur.mem[,1]-predFut[,1])^2
              + (projCur.mem[,2]-predFut[,2])^2
              + (projCur.mem[,3]-predFut[,3])^2
              + (projCur.mem[,4]-predFut[,4])^2
              + (projCur.mem[,5]-predFut[,5])^2
              + (projCur.mem[,6]-predFut[,6])^2
              + (projCur.mem[,7]-predFut[,7])^2
              + (projCur.mem[,8]-predFut[,8])^2
              + (projCur.mem[,9]-predFut[,9])^2))
}

# Calculate the GO for each GCM
genOffallGCMs <- lapply(ssp370stack, GOAcrossRange)
names(genOffallGCMs) <- str_sub(ssp370stack.names,15,-5)
...
```{r GOSSP370GFMEMPlotDens, fig.height=3,fig.width=6}
# Distribution of the genomic offset for each GCM
dfGOallGCMs %>%  
  pivot_longer(cols=everything(),names_to="model",values_to="go") %>% 
  ggplot(aes(x=go,color=model)) + 
...
```{r PlotGOSSP370GFMEM,fig.height=8,fig.width=8}
rast.go <- ssp370stack[[1]][[1]]
rasValue <- raster::extract(rast.go, PinpinDistri,cellnumbers=T)
rasValue <- as.data.frame(rasValue)
rast.go[rasValue$cell] <- dfGOallGCMs$mean
...
```{r GOSSP370GFGEO, cache=T}
# We have to project the current climate in the same resolution as the future climate. 
# So here we project the current climate in ~31km2 spatial resolution:
rasValue <- raster::extract(stackall.lowres, PinpinDistri,cellnumbers=T)
rasValue <- as.data.frame(rasValue)
rasValue <- na.omit(rasValue)
rasValue <- cbind(xyFromCell(stackall.lowres,rasValue[,1]),rasValue)
rasValue <- rasValue %>% rename(longitude=x,latitude=y)

# transform env using gf models, see ?predict.gradientForest
projCurResWorldClim <- predict(gf.geo, rasValue[,-3]) # remove cell column before transforming
...
GOAcrossRange <- function(x){
  ...
  rasValue <- as.data.frame(rasValue)
  rasValue <- na.omit(rasValue)
  rasValue <- cbind(xyFromCell(x,rasValue[,1]),rasValue)
  rasValue <- rasValue %>% rename(longitude=x,latitude=y)

  # transform env using gf models, see ?predict.gradientForest
  predFut <- predict(gf.geo, rasValue[,-3])
  
  # calculate euclidean distance between current and future genetic spaces  
  return(sqrt((projCurResWorldClim[,1]-predFut[,1])^2
              + (projCurResWorldClim[,2]-predFut[,2])^2
              + (projCurResWorldClim[,3]-predFut[,3])^2
              + (projCurResWorldClim[,4]-predFut[,4])^2
              + (projCurResWorldClim[,5]-predFut[,5])^2
              + (projCurResWorldClim[,6]-predFut[,6])^2
              + (projCurResWorldClim[,7]-predFut[,7])^2
              + (projCurResWorldClim[,8]-predFut[,8])^2
              + (projCurResWorldClim[,9]-predFut[,9])^2))
}
...
```{r GOSSP370GFGEOPlotDens, fig.height=3,fig.width=6}
# Distribution of the genomic offset for each GCM
dfGOallGCMs %>%  
  pivot_longer(cols=everything(),names_to="model",values_to="go") %>% 
  ggplot(aes(x=go,color=model)) + 
...
```{r PlotGOSSP370GFGEO,fig.height=8,fig.width=8}
rast.go <- ssp370stack[[1]][[1]]
rasValue <- raster::extract(rast.go, PinpinDistri,cellnumbers=T)
rasValue <- as.data.frame(rasValue)
rast.go[rasValue$cell] <- dfGOallGCMs$mean
