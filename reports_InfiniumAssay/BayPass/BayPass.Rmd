...
env <- merge(env,
             readRDS(file="data/FireData/DfBurnedAreabyProv.rds") %>% select(BurnedArea,prov),by=c("prov"))
...
write.table(as.data.frame(t(DFenv[4,])), file = "outputs/CandidateSNPs/baypass/EnvVariablesBayPassSandTop", sep = " ",
            row.names = FALSE, col.names = FALSE)
write.table(as.data.frame(t(DFenv[5,])), file = "outputs/CandidateSNPs/baypass/EnvVariablesBayPassBurnedArea", sep = " ",
            row.names = FALSE, col.names = FALSE)
```
...
rda.cacand <- readRDS(file="outputs/CandidateSNPs/rda/216CandSNPsRDA_AvgWater.rds")
...
outIS <- data.frame(snp=rownames(dfsum1),
                  medianBF=apply(data.frame(BF1,BF2,BF3,BF4,BF5),1,median),
                  medianBeta=apply(data.frame(beta1,beta2,beta3,beta4,beta5),1,median))

dfIS <- dfIS %>% mutate(bio1=c(length(which(outIS$medianBF>20)),
                               length(which(outIS$medianBF>10)),
                               length(which(outIS$medianBF>5))))

outIS$COVARIABLE <- "bio1"
candBaypass <- outIS[which(outIS$medianBF>threshold),]
candBaypass
```
...
dfIS <- dfIS %>% mutate(bio2=c(length(which(outIS$medianBF>20)),
                               length(which(outIS$medianBF>10)),
                               length(which(outIS$medianBF>5))))

outIS$COVARIABLE <- "bio2"
outIS[which(outIS$medianBF>threshold),]
candBaypass <- bind_rows(candBaypass,outIS[which(outIS$medianBF>threshold),])
```
...
dfIS <- dfIS %>% mutate(bio12=c(length(which(outIS$medianBF>20)),
                               length(which(outIS$medianBF>10)),
                               length(which(outIS$medianBF>5))))

outIS$COVARIABLE <- "bio12"
outIS[which(outIS$medianBF>threshold),]
candBaypass <- bind_rows(candBaypass,outIS[which(outIS$medianBF>threshold),])
```
...
dfIS <- dfIS %>% mutate(bio14=c(length(which(outIS$medianBF>20)),
                               length(which(outIS$medianBF>10)),
                               length(which(outIS$medianBF>5))))

outIS$COVARIABLE <- "bio14"
outIS[which(outIS$medianBF>threshold),]
candBaypass <- bind_rows(candBaypass,outIS[which(outIS$medianBF>threshold),])
```
...
dfIS <- dfIS %>% mutate(bio13=c(length(which(outIS$medianBF>20)),
                               length(which(outIS$medianBF>10)),
                               length(which(outIS$medianBF>5))))

outIS$COVARIABLE <- "bio13"
outIS[which(outIS$medianBF>threshold),]
candBaypass <- bind_rows(candBaypass,outIS[which(outIS$medianBF>threshold),])
```
...
dfIS <- dfIS %>% mutate(bio5=c(length(which(outIS$medianBF>20)),
                               length(which(outIS$medianBF>10)),
                               length(which(outIS$medianBF>5))))

outIS$COVARIABLE <- "bio5"
outIS[which(outIS$medianBF>threshold),]
candBaypass <- bind_rows(candBaypass,outIS[which(outIS$medianBF>threshold),])
```
...
dfIS <- dfIS %>% mutate(bio6=c(length(which(outIS$medianBF>20)),
                               length(which(outIS$medianBF>10)),
                               length(which(outIS$medianBF>5))))

outIS$COVARIABLE <- "bio6"
outIS[which(outIS$medianBF>threshold),]
candBaypass <- bind_rows(candBaypass,outIS[which(outIS$medianBF>threshold),])
```
...
dfIS <- dfIS %>% mutate(water_top=c(length(which(outIS$medianBF>20)),
                               length(which(outIS$medianBF>10)),
                               length(which(outIS$medianBF>5))))

outIS$COVARIABLE <- "water_top"
outIS[which(outIS$medianBF>threshold),]
candBaypass <- bind_rows(candBaypass,outIS[which(outIS$medianBF>threshold),])
```
...
dfIS <- dfIS %>% mutate(sand_top=c(length(which(outIS$medianBF>20)),
                               length(which(outIS$medianBF>10)),
                               length(which(outIS$medianBF>5))))

outIS$COVARIABLE <- "sand_top"
outIS[which(outIS$medianBF>threshold),]
candBaypass <- bind_rows(candBaypass,outIS[which(outIS$medianBF>threshold),])
```
...
dfIS <- dfIS %>% mutate(depth_roots=c(length(which(outIS$medianBF>20)),
                               length(which(outIS$medianBF>10)),
                               length(which(outIS$medianBF>5))))

outIS$COVARIABLE <- "depth_roots"
outIS[which(outIS$medianBF>threshold),]
candBaypass <- bind_rows(candBaypass,outIS[which(outIS$medianBF>threshold),])
```
...
dfIS <- dfIS %>% mutate(TRI=c(length(which(outIS$medianBF>20)),
                               length(which(outIS$medianBF>10)),
                               length(which(outIS$medianBF>5))))

outIS$COVARIABLE <- "TRI"
outIS[which(outIS$medianBF>threshold),]
candBaypass <- bind_rows(candBaypass,outIS[which(outIS$medianBF>threshold),])
```
...
```{bash ISAuxModelBurnedArea, eval=F}
cd ~/Documents/GenomicOffset/GenomicOffsetPinPin/outputs/CandidateSNPs/baypass/ISrunsBurnedArea

~/Bureau/baypass_2.2/sources/g_baypass \
    -gfile ../5165snps523genotypesImpNABayPass \
    -efile ../EnvVariablesBayPassBurnedArea \
    -omegafile ../anacore_mat_omega.out \
    -outprefix anacovis1
...
covis.snp.res=read.table("outputs/CandidateSNPs/baypass/ISrunsBurnedArea/anacovis1_summary_betai_reg.out",h=T)
layout(matrix(1:3,3,1))
plot(covis.snp.res$BF.dB.,xlab="SNP",ylab="BFis (in dB)",main="Estimates of the Bayes Factor")
plot(covis.snp.res$eBPis,xlab="SNP",ylab="eBPis",main="Empirical Bayesian P-value")
plot(covis.snp.res$Beta_is,xlab="SNP",ylab=expression(beta~"coefficient"),main="Estimates of the regression coefficients")
...
# Bayes factors
BF1 <- read.table("outputs/CandidateSNPs/baypass/ISrunsBurnedArea/anacovis1_summary_betai_reg.out",h=T)$BF.dB.
BF2 <- read.table("outputs/CandidateSNPs/baypass/ISrunsBurnedArea/anacovis2_summary_betai_reg.out",h=T)$BF.dB.
BF3 <- read.table("outputs/CandidateSNPs/baypass/ISrunsBurnedArea/anacovis3_summary_betai_reg.out",h=T)$BF.dB.
BF4 <- read.table("outputs/CandidateSNPs/baypass/ISrunsBurnedArea/anacovis4_summary_betai_reg.out",h=T)$BF.dB.
BF5 <- read.table("outputs/CandidateSNPs/baypass/ISrunsBurnedArea/anacovis5_summary_betai_reg.out",h=T)$BF.dB.
cor(data.frame(BF1,BF2,BF3,BF4,BF5))

# XtXst (calibrated estimator of the XtX statistic)
XtXst1 <- read.table("outputs/CandidateSNPs/baypass/ISrunsBurnedArea/anacovis1_summary_pi_xtx.out",h=T)$XtXst
XtXst2 <- read.table("outputs/CandidateSNPs/baypass/ISrunsBurnedArea/anacovis2_summary_pi_xtx.out",h=T)$XtXst
XtXst3 <- read.table("outputs/CandidateSNPs/baypass/ISrunsBurnedArea/anacovis3_summary_pi_xtx.out",h=T)$XtXst
XtXst4 <- read.table("outputs/CandidateSNPs/baypass/ISrunsBurnedArea/anacovis4_summary_pi_xtx.out",h=T)$XtXst
XtXst5 <- read.table("outputs/CandidateSNPs/baypass/ISrunsBurnedArea/anacovis5_summary_pi_xtx.out",h=T)$XtXst
cor(data.frame(XtXst1,XtXst2,XtXst3,XtXst4,XtXst5))

# Regression coefficients Beta_is
beta1 <- read.table("outputs/CandidateSNPs/baypass/ISrunsBurnedArea/anacovis1_summary_betai_reg.out",h=T)$Beta_is
beta2 <- read.table("outputs/CandidateSNPs/baypass/ISrunsBurnedArea/anacovis2_summary_betai_reg.out",h=T)$Beta_is
beta3 <- read.table("outputs/CandidateSNPs/baypass/ISrunsBurnedArea/anacovis3_summary_betai_reg.out",h=T)$Beta_is
beta4 <- read.table("outputs/CandidateSNPs/baypass/ISrunsBurnedArea/anacovis4_summary_betai_reg.out",h=T)$Beta_is
beta5 <- read.table("outputs/CandidateSNPs/baypass/ISrunsBurnedArea/anacovis5_summary_betai_reg.out",h=T)$Beta_is
cor(data.frame(beta1,beta2,beta3,beta4,beta5))
```
...
dfIS <- dfIS %>% mutate(BurnedArea=c(length(which(outIS$medianBF>20)),
                               length(which(outIS$medianBF>10)),
                               length(which(outIS$medianBF>5))))

outIS$COVARIABLE <- "BurnedArea"
outIS[which(outIS$medianBF>threshold),]
candBaypass <- bind_rows(candBaypass,outIS[which(outIS$medianBF>threshold),])
```
...
```{r SaveCandSNPsBayPass}
saveRDS(candBaypass,file=paste0("outputs/CandidateSNPs/baypass/CandSNPsBayPass.rds"))
```
...
candRDA <- readRDS(file="outputs/CandidateSNPs/rda/216CandSNPsRDA_AvgWater.rds")
length(candRDA$snp)

comcand <- candBaypass %>% 
  filter(COVARIABLE %in% c("bio1","bio2","bio12","bio14","water_top","depth_roots","TRI","BurnedArea")) %>% 
  inner_join(candRDA,by="snp") %>% 
  select(snp,COVARIABLE,medianBF,medianBeta,loading,contains("bio"),water_top,depth_roots,TRI) %>% 
  dplyr::rename(varBaypass=COVARIABLE,BF.Baypass=medianBF,beta.Baypass=medianBeta,
         beta.bio1.RDA=bio1,
         beta.bio2.RDA=bio2,
...
comcand <- candBaypass %>% 
  ...
  inner_join(candRDA,by="snp") %>% 
  select(snp,COVARIABLE,medianBF,medianBeta,loading,contains("bio"),sand_top,depth_roots,TRI) %>% 
  dplyr::rename(varBaypass=COVARIABLE,BF.Baypass=medianBF,beta.Baypass=medianBeta,
         beta.bio1.RDA=bio1,
         beta.bio2.RDA=bio2,
         beta.bio12.RDA=bio12,
