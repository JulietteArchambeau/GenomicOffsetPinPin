...
DF <- foreach(i = covar,.packages = c("rstan","tidyverse")) %dopar% { 
  
  # Dataframe in which we are going to stock the results
  DF <- data.frame(covar=i,
                   SNPset=SNPset,
                   NbDup1=NA,
                   NbDup099=NA,
                   NbSigniCand=NA,
  ...
  for(j in SNPset){
    ...
    if(!inherits(cand,"try-error")){ # some sets are empty and there are no corresponding file, so this line allows the loop not to stop if the file is not found
      ...
      ## Duplicates
      matcor <- cor(genocand[,-1], use = "pairwise.complete.obs")
      matcor[lower.tri(matcor,diag=T)] <- NA # Keep only the upper triangle of the matrix and remove values in the diagnonal
      DF[DF$covar==i&DF$SNPset==j,"NbDup1"] <- length(which(abs(matcor)==1)) # Counting the number of pairs of SNPs that have a correlation coefficient of 1
      DF[DF$covar==i&DF$SNPset==j,"NbDup099"] <-length(which(abs(matcor)>0.99)) # Counting the number of pairs of SNPs that have a correlation coefficient higher than 0.99
      ...
      for(c in colnames(genocand[,-1])){
        ...
        output[output$snp==c,"CrossZero1"] <- ifelse((output$LowCI1[output$snp==c]>0 & output$HighCI1[output$snp==c]<0) | 
                                                       (output$LowCI1[output$snp==c]<0 & output$HighCI1[output$snp==c]>0), "-" ,"*")
        ...
        output[output$snp==c,"CrossZero2"] <- ifelse((output$LowCI2[output$snp==c]>0 & output$HighCI2[output$snp==c]<0) | 
                                                       (output$LowCI2[output$snp==c]<0 & output$HighCI2[output$snp==c]>0), "-" ,"*")
      ...
      DF[DF$covar==i&DF$SNPset==j,"NbSigniCand"] <- length(output$snp[output$beta1>0&output$CrossZero1=="*"])
      ...
      DF[DF$covar==i&DF$SNPset==j,"CrossZero1SNPcount"] <-  output[1,"CrossZero1"] <- ifelse((output$LowCI1[1]>0 & output$HighCI1[1]<0) | (output$LowCI1[1]<0 & output$HighCI1[1]>0), "-" ,"*")
