...
```{r Load GenomicData}
snp.sets.codes <- c("Ref","Mer","Com")
snp.sets.names <- c("Reference SNPs","Merged candidates","Common candidates")

# List of pairwise Fst matrices scaled between 0 and 1 (for GDM analysis)
ListPairwiseFstScaled <- readRDS(file="data_DRYAD/ListPairwiseFstScaled.rds")

# List of tables of population allele frequencies (for GF analysis)
ListAlleleFrequencies <- readRDS(file="data_DRYAD/ListAlleleFrequencies.rds")
```
...
Matt Fizpatrick's personnal communication: "How to deal with spatial structure in GF is an ongoing challenge. In the *Ecology Letters* paper we proposed MEMs, but we haven't really used those much since because they are hard to interpret and tend to be correlated with the climate gradients."
...
## Datasets for the validation part 

### Formatting NFI data

```{r LoadNFIdata}
NFIdata <- read.csv("data_DRYAD/NFIdata.csv")
```

### Formatting common garden data

```{r MerginCGandPROVenvData}
# Environmental data in the common gardens ('s2' columns)
# -------------------------------------------------------

# Loading the environemntal data
data.cg <- read.csv("data_DRYAD/EnvDataCommonGarden.csv") %>%  
  setNames(c("s2.site","s2.xCoord","s2.yCoord",paste0("s2.",colnames(.)[-c(1:3)])))
...
# We replicate this dataset 34 times (one for each common garden)
data.cg.gdm <- replicate(34,data.cg,simplify=F) %>% 
  bind_rows() %>% 
  dplyr::select(-s2.site,-s2.bio1)
  

# Environmental data in the provenances ('s1' columns)
# ----------------------------------------------------
data.prov <- read.csv("data_DRYAD/PopulationPopStructureEnvData.csv") %>% 
...
> Environmental dataset for the GDM analysis.

```{r FormmatingEnvDataGDM}
data.pred.gdm <-  data.prov %>% 
  ...
  dplyr::select(distance, weights, s1.xCoord, s1.yCoord, s2.xCoord, s2.yCoord,
                everything()) %>% 
  mutate(s2.yCoord=s1.yCoord,
         s2.xCoord=s1.xCoord,
         s2.BurnedArea=s1.BurnedArea, # !! We fix BurnedArea and TRI so that they have the same values in the common gardens
         s2.TRI=s1.TRI)
...
In this dataset, there are `r nrow(data.pred.gdm)` rows, corresponding to 5 (nb common gardens) * 34 (nb pops), and `r ncol(data.pred.gdm)` variables (columns). Columns with `s1` before correspond to the environmental data of each population and columns with `s2` before correspond to the environmental data of each common garden.


> Environmental dataset for the GF analysis.

For the GF analysis, we create two separate datasets, one with the environmental data of the common gardens and one with the environmental data of the populations.

```{r FormmatingEnvDataGF}
data.pred.prov.gf <- data.pred.gdm %>% 
  dplyr::select(contains("s1"),-contains("Coord")) %>% 
  setNames(str_sub(colnames(.),4,-1))

data.pred.prov.gf[1:10,]  %>% 
  kable(digits=3) %>% 
  kable_styling(bootstrap_options = c("hover", "condensed"), full_width = F) %>%
  column_spec(1, bold = T)
...
Function to project the genomic composition across the landscape (map + PCA)
...
pdf(paste0("outputs_paper/GDM/Projections/",time,"ProjMAP_Res",res,"_",scenario,"_",snp.sets.codes[[counter]],"_GDM.pdf"), 
...
png(paste0("outputs_paper/GDM/Projections/",time,"ProjPCA_Res",res,"_",scenario,"_",snp.sets.codes[[counter]],"_GDM.png"),
...
png(paste0("outputs_paper/GDM/Projections/",time,"ProjPCA_Res",res,"_",scenario,"_",snp.sets.codes[[counter]],"_GDM_WithoutLabels.png"),
     width = 1200,height=1200,res=150)
nvs <- dim(pca$rotation)[1]
vec <-  row.names(pca$rotation)
lv <- length(vec)
...
# set 3 (3rd iteration) => common candidates
counter <- 0
...
pdf(paste0("outputs_paper/GDM/ModelRuns/Figs/ObsPred_",snp.sets.codes[[counter]],".pdf"))
...
pdf(paste0("outputs_paper/GDM/ModelRuns/Figs/Splines_",snp.sets.codes[[counter]],".pdf"))
...
p <- dfGO %>%
  ...
  ggplot(aes(x=go,color=model)) +
    ...
    xlab("Genomic offset")
ggsave(p,
       file=paste0("outputs_paper/GDM/GenomicOffset/DensityPlotGenomicOffset_",snp.sets.codes[[counter]],"_GDM.pdf"),
       device = "pdf")
...
pdf(paste0("outputs_paper/GDM/GenomicOffset/MapGenomicOffset_FixedScale_",x,"_",snp.sets.codes[[counter]],"_GDM.pdf"))
...
pdf(paste0("outputs_paper/GDM/GenomicOffset/MapGenomicOffset_VaryingScale_",x,"_",snp.sets.codes[[counter]],"_GDM.pdf"))
plot(background,col=grayscale_colors,
     axes=FALSE,
     legend=F)
plot(rast.go,box = FALSE,
...
dev.off()
  ...
  saveRDS(GO.nfi,file=paste0("outputs_paper/GDM/ExtractedGenomicOffsetNFI/nfiGO_",x,"_",snp.sets.codes[[counter]],".rds"))
...
data.pred.gdm <- data.pred.gdm %>% 
  ...
  inner_join(data.cg[,c("s2.site","s2.bio5", "s2.bio6", "s2.bio12", "s2.bio15", "s2.water_top", "s2.depth_roots", "s2.bio1")]) %>% # remove  "s2.TRI" here
  inner_join(data.prov[,c("s1.prov","s1.xCoord", "s1.yCoord","s1.bio1")])
saveRDS(data.pred.gdm,file=paste0("outputs_paper/GDM/ExtractedGenomicOffsetCG/CGGO_",snp.sets.codes[[counter]],".rds"))
...
saveRDS(outputs,file=paste0("outputs_paper/GDM/ModelRuns/Outputs/",snp.sets.codes[[counter]],".rds"))
})
```
...
pdf(paste0("outputs_paper/GF/Projections/",time,"ProjMAP_Res",res,"_",scenario,"_",snp.sets.codes[[counter]],"_GF.pdf"), 
    width=48.5, height=50)
par(mar=c(2.5,0.45,1.5,0.45)+0.1)
plot(background,
     col=grayscale_colors,
...
png(paste0("outputs_paper/GF/Projections/",time,"ProjPCA_Res",res,"_",scenario,"_",snp.sets.codes[[counter]],"_GF.png"),
     width = 1200,height=1200,res=150)
plot((pca$x[, 1:2]), 
     xlim = xrng, 
     ylim = yrng, 
...
png(paste0("outputs_paper/GF/Projections/",time,"ProjPCA_Res",res,"_",scenario,"_",snp.sets.codes[[counter]],"_GF_WithoutLabels.png"),
     width = 1200,height=1200,res=150)
plot((pca$x[, 1:2]), 
     xlim = xrng, 
     ylim = yrng, 
...
# Figures with the same scale:
pdf(paste0("outputs_paper/GF/GenomicOffset/MapGenomicOffset_FixedScale_",scenario,"_",snp.sets.codes[[counter]],"_GF.pdf"), 
    width=48.5, height=50)
par(mar=c(2.5,0.45,1.5,0.45)+0.1)
plot(background,col=grayscale_colors, 
     axes=FALSE,
...
# Figures with different scales:
pdf(paste0("outputs_paper/GF/GenomicOffset/MapGenomicOffset_VaryingScale_",scenario,"_",snp.sets.codes[[counter]],"_GF.pdf"), 
    width=48.5, height=50)
par(mar=c(2.5,0.45,1.5,0.45)+0.1)
plot(background,col=grayscale_colors, 
     axes=FALSE,
...
dev.off()
  ...
  saveRDS(GO.nfi,file=paste0("outputs_paper/GF/ExtractedGenomicOffsetNFI/nfiGO_",scenario,"_",snp.sets.codes[[counter]],".rds"))
}
```
...
#                      corr.threshold=0.50)
# To save time:
#saveRDS(gf,file=paste0("outputs_paper/GF/ModelRuns/Models/GFmodel_",snp.sets.codes[[counter]],".rds")) 
gf <- readRDS(file=paste0("outputs_paper/GF/ModelRuns/Models/GFmodel_",snp.sets.codes[[counter]],".rds"))

# Save the mean R^2 of the individual SNP models
# saveRDS(c(mean(gf$result),
#           min(gf$result),
#           max(gf$result)),file=paste0("outputs_paper/GF/ModelRuns/Outputs/",snp.sets.codes[[counter]],".rds"))

# Figures on variable and overall importance
# ------------------------------------------

# Overall importance of each variable
pdf(paste0("outputs_paper/GF/ModelRuns/Figs/OverallImportance_",snp.sets.codes[[counter]],".pdf"))
...
# Summary figure of the cumulative importance
by.importance <- names(importance(gf))
pdf(paste0("outputs_paper/GF/ModelRuns/Figs/CumulativeImportance_AllCovariates_",snp.sets.codes[[counter]],".pdf"))
plot(gf,
     plot.type = "C",
...
# -------------------------------------------------------
  ...
  inner_join(data.cg[,c("s2.site","s2.bio5", "s2.bio6", "s2.bio12", "s2.bio15", "s2.water_top", "s2.depth_roots", "s2.bio1")]) %>% # remove  "s2.TRI" here 
  inner_join(data.prov[,c("s1.prov","s1.xCoord", "s1.yCoord","s1.bio1")])
  saveRDS(data.pred.gdm,file=paste0("outputs_paper/GF/ExtractedGenomicOffsetCG/CGGO_",snp.sets.codes[[counter]],".rds"))
...
```{r TableGDMPerf}
set.names <- c("Ref","Mer","Com")

out <- set.names %>% sapply(function(x){
  
  
  out.list <- readRDS(paste0("outputs_paper/GDM/ModelRuns/Outputs/",x,".rds"))
...
print(xtable(out, type = "latex",digits=2), 
      file = "outputs_paper/GDM/ModelRuns/Outputs/GDMperformance.tex", 
...
out <- set.names %>% sapply(function(x){
  ...
  readRDS(paste0("outputs_paper/GDM/ModelRuns/Outputs/",x,".rds")) %>% 
    extract2("splines") %>% 
  ...
  bind_rows(.id="SNPSets") %>% 
  mutate(SNPSets=factor(SNPSets,levels=c("Ref","Mer","Com")))


limits <- c(0,0.8)
size.x.axis <- 14
size.text.legend <- 17
size.title.legend <- 20

p1 <- out %>% ggplot(aes(x=Geographic_x, y=Geographic_y, color=SNPSets)) +
  geom_line(size=3) +
  scale_color_manual(values=c("#5e3c99","#fdb863","#e66101" ),
                     labels=c("Reference SNPs","Merged candidates","Common candidates")) +
  xlab("Geographical distance") + 
...
# For latex:
ggsave(plot=fig,
       filename="outputs_paper/GDM/ModelRuns/Figs/SummaryISplines.pdf",
       height = 12,
       width=20,
       device="pdf")
...
# For google docs:
ggsave(plot=fig,
       filename="outputs_paper/GDM/ModelRuns/Figs/SummaryISplines.png",
       height = 12,
       width=20)
...
out <- sapply(set.names, function(x){
  readRDS(paste0("outputs_paper/GF/ModelRuns/Outputs/",x,".rds"))
}) %>% 
  as.data.frame() %>% 
  t() %>% 
  as.data.frame() %>% 
...
# Generate the latex table
print(xtable(out, type = "latex",digits=2), 
      file = "outputs_paper/GF/ModelRuns/Outputs/GFperformance.tex")
```
