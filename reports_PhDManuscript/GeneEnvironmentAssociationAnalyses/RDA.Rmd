...
```{r LoadGenomicData}
gen.imp <- readRDS(file="data/GenomicData/FormattedData/ImputedGenomicData_454clones_9817snps.rds") %>% 
  t() %>% 
  as.data.frame()
```
...
```{r RmOLO17, echo=F,eval=F}
# Only if we use the dataset: ImputedGenomicData_454clones_9817snps.rds
...
Climatic variables:
  ...
  - `bio15`: precipitation seasonality (coefficient of variation) = standard deviation of weekly or monthly precipitation values as a percentage of the mean of those values. 
...
print(xtable(DF, type = "latex",digits=2), 
      file = "outputs/CandidateSNPs/rda/NumberCandidatesForEachVariable.tex", 
...
print(xtable(sum.tab.RDA, type = "latex",digits=2), 
      file = "outputs/CandidateSNPs/rda/PartitioningVarianceRDA.tex", 
      include.rownames=FALSE)
```
