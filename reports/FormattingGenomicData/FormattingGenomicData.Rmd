...
data <- read_excel("data/GenomicData/data_curated_Affx+Inf.xlsx", sheet = 3, col_names = FALSE)
```
...
<span style="color: red;">2146 pairs of SNPs have a correlation coefficient of 1.</span>
...
**<span style="color: red;">`r nrow(tab[tab$freq.NAs<20,])` SNPs have the percentage of missing data lower than 20% and `r nrow(tab[tab$freq.NAs<15,])` lower than 15%.</span>**

```{r,echo=F}
snp20 <- tab[tab$freq.NAs<20,]$snp
snp15 <- tab[tab$freq.NAs<15,]$snp
```
...
```{r}
subdf <- df[!(row.names(df) %in% snpMAF),]       # removing MAF < 1% 
subdf <- subdf[(row.names(subdf) %in% snp20),]   # removing missing data per SNP > 20%
```

**<span style="color: red;">After removing SNPs with MAF < 1% and missing data > 20%, `r nrow(subdf)` SNPs left.</span>**

```{r}
subdf <- df[!(row.names(df) %in% snpMAF),]       # removing MAF < 1% 
subdf <- subdf[(row.names(subdf) %in% snp15),]   # removing missing data per SNP > 15%
```

**<span style="color: red;">After removing SNPs with MAF < 1% and missing data > 15%, `r nrow(subdf)` SNPs left.</span>**