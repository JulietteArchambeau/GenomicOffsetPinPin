...
**<span style="color: red;">`r nrow(tab[tab$freq.NAs<20,])` clones have the percentage of missing data lower than 20%, `r nrow(tab[tab$freq.NAs<18,])` lower than 18% and `r nrow(tab[tab$freq.NAs<15,])` lower than 15%.</span>**
...
We are going to keep the `r nrow(tab[tab$freq.NAs<threshold.NA.per.clone,])` clones that have less than `r threshold.NA.per.clone`% missing data. 
...
#tab.clon <- table(tab.clon$assay)
table(tab.clon$assay)
```


<!-- Among the `r ncol(df)` clones used in the present paper (after filtering), **`r tab.clon[[1]]` were genotyped in both the Affymetrix and Infinium assays** and **`r tab.clon[[2]]` clones were only genotyped by the Affymetrix assay**. -->

All clones were genotyped with both the Affymetrix and Infinium assays.

We want the average number of missing data per tree:
...
tab[c(1:10,(nrow(tab)-5):nrow(tab)),] %>%
  kable() %>%  
  kable_styling(font_size=11,
                bootstrap_options = c("stripped","hover", "condensed"), full_width = F)
...
We load the dataset in which there is the proportion of belonging of each genotype to a given gene pool.
...
We check whether some clones in the genomic dataset have no population structure data.

```{r IdentifyClonesNoPopStructure}
setdiff(colnames(df),prop$clon)
```

One clone, OLO17, has no structure data. If we look at the other clones of the OLO population, all mainly belong to the French Atlantic gene pool (`Q4`):
...
Comment: if the threshold of missing data per clone is 18% (or 15%), then we don't have to care about OLO17 as it is not included in the selected clones.

```{r MergePopStructureWithGenomicData}
mainGP <- prop %>% dplyr::select(clon,max.Q) # extract the main gene pool for each clone.

mainGP[nrow(mainGP) + 1,] <- c("OLO17","Q4") # we add the main  gene pool of OLO17
