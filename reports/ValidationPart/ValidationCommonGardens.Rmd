...
### Survival data (response variable)
...
ggsave(p,file=paste0("outputs/ValidationCG/Figs/MortalityModels/SummaryFigures/",index.match[[index]][[1]],".pdf"),device="pdf")
})
```
...
list.files(paste0("outputs/ValidationCG/Figs/MortalityModels/LOOs"),full.names=TRUE) %>% 
  str_subset(sel.site) %>% 
  sapply(readRDS, USE.NAMES = TRUE,simplify=FALSE) %>% 
  loo_compare() %>% 
  as.data.frame() %>% 
  ...
  mutate(site=sel.site,
         variable=str_sub(variable,52,-13))
