...
```{r TransformingBeforeProjectionGFCurrentClimate,cache=TRUE}
...
```{r BiplotGFCurrentClimate, echo=F,fig.height=6,fig.width=6, cache=T}
nvs <- dim(pca$rotation)[1]
vec <- names(gf$X)[5:10]
lv <- length(vec)
vind <- rownames(pca$rotation) %in% vec
...
```{r BiplotGFCurrentClimateLatLong, echo=F,fig.height=6,fig.width=6}
nvs <- dim(pca$rotation)[1]
vec <- names(gf$X)
lv <- length(vec)
vind <- rownames(pca$rotation) %in% vec
...
```{r TransformingBeforeProjectionGFFutureClimateSSP370LatLong}
# Load model estimating on the test lat/long set of variables
#gf <- readRDS(file=paste0("../../outputs/GF/models/GFAvgWaterGeo.rds"))
...
```{r ProjectionGFFutureClimateSSP370LatLong, fig.height=8, fig.width=8}
##assigns to colors, edit as needed to maximize color contrast, etc.
a1 <- pca$x[,1]; a2 <- pca$x[,2]; a3 <- pca$x[,3]
r <- a1+a2; g <- -a2; b <- a3+a2-a1
...
```{r BiplotGFFutureClimateSSP370LatLong, echo=F,fig.height=6,fig.width=6}
nvs <- dim(pca$rotation)[1]
vec <- names(gf$X)
lv <- length(vec)
vind <- rownames(pca$rotation) %in% vec
...
# transform env using gf models, see ?predict.gradientForest
projCurResWorldClim <- predict(gf, rasValue[,-3]) # remove cell column before transforming
...
```{r GOSSP370GFPlotDens, fig.height=3,fig.width=6}
# Distribution of the genomic offset for each GCM
dfGOallGCMs %>%  
  pivot_longer(cols=everything(),names_to="model",values_to="go") %>% 
  ggplot(aes(x=go,color=model)) + 
...
```{r PlotGOSSP370GF,fig.height=8,fig.width=8}
rast.go <- ssp370stack[[1]][[1]]
rasValue <- raster::extract(rast.go, PinpinDistri,cellnumbers=T)
rasValue <- as.data.frame(rasValue)
rast.go[rasValue$cell] <- dfGOallGCMs$mean
