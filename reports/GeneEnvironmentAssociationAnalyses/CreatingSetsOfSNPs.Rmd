...
```{r ListSNPs}
snp.set.names <- c("Reference SNPs",
                   "Merged candidates",
...
```{r LoadCandBayPass}
candBaypass <- readRDS(file="outputs/CandidateSNPs/baypass/CandSNPsBayPass.rds")
candBaypass  %>% 
  kable() %>% 
...
mergedcand <- candBaypass %>% 
  filter(COVARIABLE %in% c("bio13","bio5","bio6","bio14","water_top","depth_roots","TRI","BurnedArea")) %>% 
...
highcand <- candBaypass %>% 
  filter(COVARIABLE %in% c("bio13","bio5","bio6","bio14","water_top","depth_roots","TRI","BurnedArea")) %>% 
  full_join(candRDA,by="snp") %>% 
  filter(abs(medianBeta) >0 |abs(bio13)>0.3 | abs(bio5)>0.3 | abs(bio6)>0.3 | abs(bio14)>0.3 | abs(water_top)>0.3 | abs(depth_roots)>0.3 | abs(BurnedArea)>0.3 | abs(TRI)>0.3) %>% 
  select(snp,COVARIABLE,medianBF,medianBeta,loading,contains("bio"),water_top,depth_roots,BurnedArea,TRI) %>% 
  dplyr::rename(varBaypass=COVARIABLE,
         BF.Baypass=medianBF,
         beta.Baypass=medianBeta,
...
snp.sets <- list(rownames(geno), # Reference SNPs
                 rownames(geno), # Merged candidates
                 rownames(geno), # Candidates under expected strong selection
                 rownames(geno)) # Common candidates

names(snp.sets) <- c("Ref","Mer","Mid","Com")
 saveRDS(snp.sets,file="outputs/ListSetsSNPs.rds")
names(snp.sets) <- snp.set.names
```
